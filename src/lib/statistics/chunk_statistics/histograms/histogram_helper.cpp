...
std::string previous_value(const std::string& value, const std::string& supported_characters,
  ...
  // Special case, there is no previous value.
  if (value.empty()) {
    return "";
  }
  ...
  return substring + static_cast<char>(last_char - 1) +
         std::string(string_prefix_length - cleaned_value.length(), supported_characters.back());
}
...
uint64_t convert_string_to_number_representation(const std::string& value, const std::string& supported_characters,
  ...
  return char_value + convert_string_to_number_representation(trimmed.substr(1, trimmed.length() - 1),
                                                              supported_characters, string_prefix_length - 1);
}
