...
PausableLoopThread::PausableLoopThread(std::chrono::nanoseconds loop_sleep_time,
  ...
  _loop_thread = std::thread([&, loop_func] {
    ...
    while (!_shutdown_flag) {
      ...
      if (_loop_sleep_time > std::chrono::nanoseconds(0)) {
...
void PausableLoopThread::set_loop_sleep_time(std::chrono::nanoseconds loop_sleep_time) {
  _loop_sleep_time = loop_sleep_time;
}
