 ...
 */
 // changed COUNT(*) to COUNT(l_discount)
const char* const tpch_query_1 =
    R"(SELECT l_returnflag, l_linestatus, SUM(l_quantity) as sum_qty, SUM(l_extendedprice) as sum_base_price,
      ...
      AVG(l_extendedprice) as avg_price, AVG(l_discount) as avg_disc, COUNT(l_discount) as count_order
      FROM lineitem
...
const char* const tpch_query_10 =
    R"(SELECT c_custkey, c_name, SUM(l_extendedprice * (1.0 - l_discount)) as revenue, c_acctbal, n_name, c_address, c_phone, c_comment FROM customer, orders, lineitem, nation WHERE c_custkey = o_custkey AND l_orderkey = o_orderkey AND o_orderdate >= '1993-10-01' AND o_orderdate < '1994-01-01' AND l_returnflag = 'R' AND c_nationkey = n_nationkey GROUP BY c_custkey, c_name, c_acctbal, c_phone, n_name, c_address, c_comment ORDER BY revenue DESC;)";
