...
TEST_F(ProjectionNodeTest, ColumnReferenceByNamedColumnReference) {
  EXPECT_EQ(_projection_node->get_column({"c", std::experimental::nullopt}), _c);
  EXPECT_EQ(_projection_node->get_column({"c", "t_a"}), _c);
  EXPECT_EQ(_projection_node->get_column({"a", std::experimental::nullopt}), _a);
  EXPECT_EQ(_projection_node->get_column({"a", "t_a"}), _a);
  EXPECT_EQ(_projection_node->get_column({"alias_for_b", std::experimental::nullopt}), _b);
  EXPECT_EQ(_projection_node->find_column({"alias_for_b", "t_a"}), std::experimental::nullopt);
  EXPECT_EQ(_projection_node->get_column({"some_addition", std::experimental::nullopt}), _some_addition);
  EXPECT_EQ(_projection_node->find_column({"some_addition", "t_a"}), std::experimental::nullopt);
}
