...
for old, new in zip(old_data['benchmarks'], new_data['benchmarks']):
	...
	rows = [
		("items / s", float(old['items_per_second']), float(new['items_per_second']), True),
		("avg iteration", int(old['avg_real_time_per_iteration']), int(new['avg_real_time_per_iteration']), False),
		("prepare operators", old_prepare, new_prepare, False),
		("execute operators", old_execute, new_execute, False),
		("total operators", old_total, new_total, False)
	]
	table = AsciiTable(table_data)
